...
S.tex17 = uicontrol(S.fh,'Style','text',...
    'String','Pencil Sketch    ','FontSize',12,...
    'backgroundcolor',[0.1 0.1 0.1],...
...
S.tex18 = uicontrol(S.fh,'Style','text',...
    'String','Color Sketch    ','FontSize',12,...
    'backgroundcolor',[0.1 0.1 0.1],...
    'Foregroundcolor',[0.5 0.5 1],...
...
if strcmp(get(S.ed12, 'string'), '')
    ...
    %====main plot====%
    I = imread(get(S.ed12,'string'));
    if max(size(imread(get(S.ed12,'string')))) > 1024
        I = imresize(I, 1024/max(size(I)));
    end
    imwrite(pencil_draw(I,...
        get(S.sl1,'Value'),...
        get(S.sl2,'Value'),...
        get(S.sl3,'Value'),...
        get(S.sl4,'Value'),...
...
function [] = pb4_call(varargin)  %====Save file
...
function [] = pb6_call(varargin)  %====Save file
[filename, pathname, filterindex] = uiputfile({'*.bmp;*.jpg;*.tif;*.png;*.gif','All Image Files';...
    '*.*','All Files' },'Save Image');
if ~isequal( [filename, pathname, filterindex], [0,0,0])
